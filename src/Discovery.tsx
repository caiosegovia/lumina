...
function Shelf({
  ...
  return (
    <section className="discovery-shelf">
      <div className="discovery-shelf-heading">
        <div>
          <h3>{title}</h3>
          <p>{description}</p>
        </div>
              ...
              <header>
                <div>
                  ...
                  {group.recommendation && (
                    <small>{group.recommendation}</small>
                  )}
                </div>
                <div className="discovery-group-actions">
                  ...
                  {onRename && group.placeKey && (
                    <button onClick={() => onRename(group)}>Renomear</button>
                  )}
              ...
              <div className="discovery-strip">
                {group.items.map((item) => (
                  <Thumb
                    key={item.id}
                    item={item}
                    recommended={item.id === group.recommendedId}
                    onOpen={() => open(item)}
                  />
                ))}
              </div>
...
export default function Discovery({
  ...
  const index = async () => {
    ...
    try {
      const result = await api.buildDiscoveryIndex();
      setMessage(
        `${result.indexed} imagens analisadas · ${result.skipped} indisponíveis · ${result.failed} falhas`,
      );
      await load();
    } catch (error) {
  ...
  const savePlace = async () => {
    if (!editing?.placeKey) return;
    try {
      await api.renameLocation(editing.placeKey, placeName);
      setEditing(undefined);
      setMessage("Nome do lugar atualizado somente no catálogo.");
      await load();
    } catch (error) {
      setMessage(String(error));
    }
  ...
  const filter = (groups: DiscoveryGroup[]) => {
    const term = query.trim().toLocaleLowerCase("pt-BR");
    return term
      ? groups.filter((group) =>
          `${group.title} ${group.detail} ${group.items.flatMap((item) => [item.filename, item.camera, ...(item.visualLabels || [])]).join(" ")}`
  ...
  return (
    <div className="discovery">
      ...
      <Shelf
        ...
        groups={filter(data.memories)}
        empty="As memórias aparecerão conforme sua linha do tempo crescer."
        navigate={navigate}
      />
      ...
      {editing && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Renomear lugar"
        >
          <div>
            <h3>Nomear lugar</h3>
            <p>
              Este nome vale para mídias próximas e fica apenas no catálogo.
            </p>
            <input
              aria-label="Nome personalizado do lugar"
              autoFocus
              value={placeName}
            ...
            <footer>
              ...
              <button className="primary" onClick={savePlace}>
                Salvar nome
              </button>
            </footer>
          </div>
