...
fn read_details(conn: &rusqlite::Connection, asset_id: &str) -> Result<AssetDetails, String> {
    conn.query_row("SELECT a.camera,t.detected_format,t.mime,t.container,t.codec,t.audio_codec,t.frame_rate,t.bitrate,t.pixel_format,t.lens,t.iso,t.aperture,t.exposure,t.focal_length,t.orientation,t.color_profile,t.support_level,t.inventory_state,t.inventory_error,t.enriched_at,COALESCE(o.display_name,c.display_name),d.sublocation,d.city,d.region,d.country,d.source,CASE WHEN o.display_name IS NOT NULL OR d.source='embedded' THEN 'exact' WHEN d.source='offline' AND COALESCE(d.accuracy_m,99999)<=1000 THEN 'probable' ELSE 'approximate' END,d.altitude,d.accuracy_m FROM assets a LEFT JOIN asset_technical_metadata t ON t.asset_id=a.id LEFT JOIN asset_locations al ON al.asset_id=a.id LEFT JOIN location_cells c ON c.cell_key=al.cell_key LEFT JOIN location_overrides o ON o.cell_key=al.cell_key LEFT JOIN asset_location_details d ON d.asset_id=a.id WHERE a.id=?1",[asset_id],|row|Ok(AssetDetails{
        camera:row.get(0)?,detected_format:row.get(1)?,mime:row.get(2)?,container:row.get(3)?,codec:row.get(4)?,audio_codec:row.get(5)?,frame_rate:row.get(6)?,bitrate:row.get(7)?,pixel_format:row.get(8)?,lens:row.get(9)?,iso:row.get(10)?,aperture:row.get(11)?,exposure:row.get(12)?,focal_length:row.get(13)?,orientation:row.get(14)?,color_profile:row.get(15)?,support_level:row.get(16)?,inventory_state:row.get(17)?,inventory_error:row.get(18)?,enriched_at:row.get(19)?,place_name:row.get(20)?,sublocation:row.get(21)?,location_city:row.get(22)?,location_region:row.get(23)?,location_country:row.get(24)?,location_source:row.get(25)?,location_confidence:row.get(26)?,altitude:row.get(27)?,location_accuracy_m:row.get(28)?
