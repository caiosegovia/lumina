...
$release = Join-Path $workspace "src-tauri\target-0.9.1-release\release"
$final = Join-Path $artifacts "Lumina-0.9.1-portable-windows-x64"
$zip = "$final.zip"
$staging = Join-Path $artifacts (".staging-Lumina-0.9.1-" + [guid]::NewGuid().ToString("N"))
if ((Test-Path $final) -or (Test-Path $zip)) { throw "A entrega 0.9.1 já existe; recuso substituição parcial." }
...
try {
  ...
  for ($attempt = 1; $attempt -le 20; $attempt++) {
    try { Rename-Item -LiteralPath $staging -NewName (Split-Path $final -Leaf) -ErrorAction Stop; $promoted = $true; break }
    catch { if ($attempt -eq 20) { throw }; Start-Sleep -Seconds 1 }
  }
  if (-not $promoted) { throw "Não foi possível promover o pacote validado." }
