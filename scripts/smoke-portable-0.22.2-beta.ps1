...
$zip = (Resolve-Path (Join-Path $workspace "artifacts\Lumina-0.22.2-beta.1-portable-windows-x64.zip")).Path
$extract = Join-Path $env:TEMP ("Lumina-0222b1-Smoke-" + [guid]::NewGuid().ToString("N"))
...
if($app.HasExited-or$app.MainWindowHandle-eq 0-or-not$app.Responding){throw "Frontend nao ficou pronto"}
$workingSet=$app.WorkingSet64;$null=$app.CloseMainWindow();if(-not$app.WaitForExit(10000)){$app.Kill();throw "Encerramento anormal"}
$marker=Join-Path $data "Lumina\diagnostics\running.session";if(Test-Path $marker){throw "Sessao nao limpa"}
...
$removed=$false;for($attempt=0;$attempt-lt 30-and-not$removed;$attempt++){try{Remove-Item -LiteralPath $resolved -Recurse -Force;$removed=$true}catch{Start-Sleep -Milliseconds 500}}
if(-not$removed){throw "O aplicativo passou no smoke, mas o diretorio temporario permaneceu bloqueado: $resolved"}
$result
