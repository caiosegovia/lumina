...
$zip = (Resolve-Path (Join-Path $workspace "artifacts\Lumina-0.16.0-beta.1-portable-windows-x64.zip")).Path
$extract = Join-Path $env:TEMP ("Lumina-016b1-Smoke-" + [guid]::NewGuid().ToString("N"))
...
$resolved=(Resolve-Path $extract).Path;$temp=(Resolve-Path $env:TEMP).Path;if(-not$resolved.StartsWith($temp,[StringComparison]::OrdinalIgnoreCase)-or$resolved-eq$temp){throw "Limpeza recusada"};Remove-Item -LiteralPath $resolved -Recurse -Force
$result
