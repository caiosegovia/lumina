...
$zip = (Resolve-Path (Join-Path $workspace "artifacts\Lumina-0.18.5-beta.1-portable-windows-x64.zip")).Path
$extract = Join-Path $env:TEMP ("Lumina-0185b1-Smoke-" + [guid]::NewGuid().ToString("N"))
Expand-Archive -LiteralPath $zip -DestinationPath $extract
$manifest = Get-Content (Join-Path $extract "MANIFEST.json") -Raw | ConvertFrom-Json
foreach ($entry in $manifest) {$path=Join-Path $extract $entry.path.Replace("/", "\");if(-not(Test-Path -LiteralPath $path)){throw "Ausente: $($entry.path)"};if((Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $entry.sha256){throw "Hash invÃ¡lido: $($entry.path)"}}
...
if($app.HasExited-or$app.MainWindowHandle-eq 0-or-not$app.Responding){throw "Frontend nÃ£o ficou pronto"}
$workingSet=$app.WorkingSet64;$null=$app.CloseMainWindow();if(-not$app.WaitForExit(10000)){$app.Kill();throw "Encerramento anormal"}
$marker=Join-Path $data "Lumina\diagnostics\running.session";if(Test-Path $marker){throw "SessÃ£o nÃ£o limpa"}
...
if(-not$removed){throw "O aplicativo passou no smoke, mas o diretÃ³rio temporÃ¡rio permaneceu bloqueado: $resolved"}
$result
